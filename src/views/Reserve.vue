<template>
  <div class="lab-booking">
    ...
    <el-form :model="filters" label-width="100px">
      ...
      <el-button type="primary" @click="fetchUserReservations">我的预约</el-button>
    </el-form>
    ...
    <div class="user-reservations" v-if="!showAvailableLabs">
      <h3>我的预约</h3>
      <el-card
      ...
      >
        <div class="lab-content">
          ...
          <p>时段: {{ getPeriodLabel(reservation.periodId) }}</p>
        </div>
        <div class="lab-actions">
          <el-button type="danger" @click="cancelReservation(reservation.id)">取消预约</el-button>
        </div>
      </el-card>
    </div>
...
const days = ref(['星期一', '星期二', '星期三', '星期四', '星期五', '星期六', '星期日']); // 星期映射为1到7
...
// 时间段映射
const timeSlotMap = timeSlots.value.reduce((map, slot) => {
  map[slot.id] = slot.label;
  return map;
}, {});

const getPeriodLabel = (periodId) => {
  return timeSlotMap[periodId] || '未知时段'; // 返回对应的label或默认值
};


const getDay = (day) => {
  return dayyMap[dayId] || "未知时间";
}
...
const showAvailableLabs = ref(true); // 控制显示可用实验室或我的预约
...
const fetchCourses = async () => {
  try {
    ...
    const response = await axios.get(`http://127.0.0.1:8080/api/course/teacher/${userId}`);
...
const fetchAvailableLabs = async () => {
  if (
    ...
    try {
      ...
      // 调用获取可用实验室的 API
      const response = await axios.get('http://127.0.0.1:8080/api/lab/available', {
...
const bookLab = async (lab) => {
  ...
  try {
    const response = await axios.post('http://127.0.0.1:8080/api/reservation/add', reservationData);
...
const fetchUserReservations = async () => {
  try {
    ...
    const response = await axios.get(`http://127.0.0.1:8080/api/reservation/user/${userId}`);
    userReservations.value = response.data; // 更新预约数据
    console.log(userReservations.value)
    showAvailableLabs.value = !showAvailableLabs.value; // 切换到我的预约视图
  } catch (error) {
...
const cancelReservation = async (reservationId) => {
  try {
    await axios.delete(`http://127.0.0.1:8080/api/reservation/${reservationId}`); // 调用取消预约的 API
    alert('预约已取消');
