  ...
  <div class="course-management">
      ...
      >
        <div class="card-header">
          ...
          <div>
            ...
            <el-button type="text" @click.stop="editCourse(course)">编辑</el-button>
            <el-button type="text" @click.stop="confirmDeleteCourse(course.id)">删除</el-button>
          </div>
        </div>
    ...
    <el-dialog title="添加课程" v-model="addCourseDialogVisible">
      <el-form :model="newCourse" label-width="100px">
        ...
        <el-form-item label="学生人数" :rules="[{ required: true, message: '请输入学生人数', trigger: 'blur' }]">
    ...
    <el-dialog title="编辑课程" v-model="editCourseDialogVisible">
      <el-form :model="currentCourse" label-width="100px">
        ...
        <el-form-item label="授课教师ID" :rules="[{ required: true, message: '请输入授课教师ID', trigger: 'blur' }]">
          <el-input v-model="currentCourse.teacherId" type="number"></el-input>
        </el-form-item>
        <el-form-item label="学生人数" :rules="[{ required: true, message: '请输入学生人数', trigger: 'blur' }]">
...
const newCourse = ref({ name: '', className: '', studyHour: 0, teacherId: 0, stuNumber: 0 }) // 添加学生人数字段
...
const showAddCourseDialog = () => {
  newCourse.value = { name: '', className: '', studyHour: 0, teacherId: 0, stuNumber: 0 } // 重置课程信息
...
const addCourse = async () => {
  try {
    if (!newCourse.value.name || !newCourse.value.className || newCourse.value.studyHour === 0 || newCourse.value.teacherId === 0 || newCourse.value.stuNumber === 0) {
      alert('请填写完整课程信息')
      return
    } 
    try {
      newCourse.value.teacherId = parseInt(newCourse.value.teacherId);
      newCourse.value.stuNumber = parseInt(newCourse.value.stuNumber); // 确保学生人数为数字
    } catch {
      alert('授课教师ID和学生人数必须为数字')
      return
    }
    ...
    const courseData = {
      ...
      stuNumber: newCourse.value.stuNumber, // 添加学生人数
...
const updateCourse = async () => {
  try {
    const response = await axios.put(`http://127.0.0.1:8080/api/course/${currentCourse.value.id}`, currentCourse.value);
...
const fetchCourses = async () => {
  try {
    const response = await axios.get(`http://127.0.0.1:8080/api/course/teacher/${teacherId.value}`) // 替换为后端地址
